...
- Events with `$0` pricing will not get `stripePriceId` values from Stripe (expected — Stripe doesn't handle free transactions). The `filterValidPrices` utility strips prices without `stripePriceId`, causing free events to show "No pricing available" on the public detail page.
