 ...
 */
function buildTestPrompt(spec: TestSpec): string {
	return `You are an E2E test runner. Run the following test using agent-browser CLI.

## Test Specification
- **Goal:** ${spec.goal}
- **Start URL:** ${config.baseUrl}${spec.startUrl}
 ...
 */
async function callCursorAgent(prompt: string): Promise<string> {
...
export async function runWorker(specPath: string): Promise<TestResult> {
	...
	const result = await callCursorAgent(prompt);
...
if (import.meta.url === `file://${process.argv[1]}`) {
	...
	if (!specPath) {
		console.error("Usage: bun run scripts/test-worker.ts <spec-path> [--debug]");
		console.error("  --debug  Stream agent output in real-time");
		process.exit(1);
	}

	if (debugMode) {
		console.log("🔍 Debug mode enabled - streaming agent output\n");
	}
